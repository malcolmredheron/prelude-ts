 ...
 * @param T the item type
 */
export interface LinkedList<T> extends Seq<T> {
    /**
 ...
 * @param T the item type
 */
export class EmptyLinkedList<T> implements LinkedList<T> {
     ...
     */
    prepend(elt: T): ConsLinkedList<T> {
        return new ConsLinkedList(elt, this.asLinkedList());
    }
 ...
 * @param T the item type
 */
export class ConsLinkedList<T> implements LinkedList<T> {
    ...
    get(idx: number): Option<T> {
        let curItem: LinkedList<T> = this;
        let i=0;
        while (LinkedList.isNotEmpty(curItem)) {
            if (i === idx) {
    ...
    take(n: number): LinkedList<T> {
        ...
        let curItem: LinkedList<T> = this;
        let i = 0;
        while (i++ < n && (LinkedList.isNotEmpty(curItem))) {
            result = new ConsLinkedList(curItem.value, result);
            curItem = curItem._tail;
        }
        return result.reverse();
    ...
    takeWhile(predicate: (x:T)=>boolean): LinkedList<T> {
        let result = <LinkedList<T>><EmptyLinkedList<T>>emptyLinkedList;
        let curItem: LinkedList<T> = this;
        while (LinkedList.isNotEmpty(curItem) && predicate(curItem.value)) {
            result = new ConsLinkedList(curItem.value, result);
            curItem =curItem._tail;
        }
        return result.reverse();
    ...
    drop(n:number): LinkedList<T> {
        let i = n;
        let curItem: LinkedList<T> = this;
        while (i-- > 0 && LinkedList.isNotEmpty(curItem)) {
            curItem = curItem._tail;
        }
        return curItem;
    }
    ...
    zip<U>(other: Iterable<U>): LinkedList<[T,U]> {
        ...
        while ((LinkedList.isNotEmpty(curItem)) && (!otherCurItem.done)) {
            result = new ConsLinkedList(
    ...
    splitAt(index:number): [LinkedList<T>,LinkedList<T>] {
        ...
        let curItem: LinkedList<T> = this;
        let i = 0;
        while (i++ < index && (LinkedList.isNotEmpty(curItem))) {
            first = new ConsLinkedList(curItem.value, first);
            curItem = curItem._tail;
        }
        return [first.reverse(), curItem];
    ...
    append(v:T): ConsLinkedList<T> {
        return new ConsLinkedList(
            this.value,
            this._tail.append(v).asLinkedList());
    }
    ...
    prependAll(elts: Iterable<T>): LinkedList<T> {
        ...
        while (LinkedList.isNotEmpty(leftToAdd)) {
    ...
    scanLeft<U>(init:U, fn:(soFar:U,cur:T)=>U): LinkedList<U> {
        ...
        while (LinkedList.isNotEmpty(curItem)) {
            ...
            result = new ConsLinkedList(soFar, result.asLinkedList());
            curItem = curItem._tail;
        }
        return result.reverse();
    }
    ...
    equals(other: LinkedList<T&WithEquality>): boolean {
        ...
        while (true) {
            ...
            if (!LinkedList.isNotEmpty(myVal) || !LinkedList.isNotEmpty(hisVal)) {
                // they are both empty, end of the stream
                // The second clause above is just to keep the type narrower happy
                return true;
            }
    ...
    toString(): string {
        ...
        while (LinkedList.isNotEmpty(curItem)) {
            ...
            curItem = tail;
            if (LinkedList.isNotEmpty(curItem)) {
                result += ", ";
            }
        }
